...
#include "caffe/layers/weighted_sigmoid_cross_entropy_loss_layer1.hpp"
...
__global__ void WeightedSigmoidCrossEntropyLossForwardGPU(const int nthreads,
          const Dtype* input_data, const Dtype* target, const Dtype* sample_weight, Dtype* loss,
  ...
  CUDA_KERNEL_LOOP(i, nthreads) {
    ...
    if (has_ignore_label_ && target_value == ignore_label_) {
      ...
      loss[i] = sample_weight[i] * (input_data[i] * (target[i] - (input_data[i] >= 0)) -
...
void WeightedSigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  WeightedSigmoidCrossEntropyLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
      CAFFE_CUDA_NUM_THREADS>>>(count, input_data, target, sample_weight, loss_data,
